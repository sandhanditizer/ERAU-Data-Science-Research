...
t_pcs = 30 # Largest number of components that the algorithm will calculate.
type = 'rand'
...
for a in range(0, MoR):
    ...
    Pu_RMSE.append([0] * t_pcs)
    Pu_R2.append([0] * t_pcs)
    HNO3_RMSE.append([0] * t_pcs)
    HNO3_R2.append([0] * t_pcs)
        
    for b in range(0, t_pcs):
        
        pca = PCA(n_components = b+1)
        
        transform_train = pca.fit_transform(data.absorb_train)
        transform_test = pca.transform(data.absorb_test)
        
        LR_model = LinearRegression()
        LR_model.fit(transform_train, data.concen_train)
        predicted_concen = LR_model.predict(transform_test)
...
plt.plot(X, Pu_RMSE_mean, color='blue', label='Plutonium (IV)', zorder=2)
plt.errorbar(X, Pu_RMSE_mean, yerr=Pu_RMSE_std, ecolor='indianred', label='Standard Deviation', color='blue')
plt.xlabel('Principal Components')
plt.ylabel('Root Mean Squared Error (M)')
plt.title(f'RMSE vs. Principal Components - {MoR} MoR - {type}')
plt.legend()
plt.grid(axis="x")
plt.show()
...
plt.subplots(figsize=(9,6))
plt.plot(X, HNO3_RMSE_mean, color='orange', label='Nitric Acid', zorder=2)
plt.errorbar(X, HNO3_RMSE_mean, yerr=HNO3_RMSE_std, ecolor='indianred', label='Standard Deviation', color='orange')
plt.xlabel('Principal Components')
plt.ylabel('Root Mean Squared Error (M)')
plt.title(f'RMSE vs. Principal Components - {MoR} MoR - {type}')
plt.legend()
...
# Comparison R2 plots.
plt.subplots(figsize=(9,6))
plt.plot(X, HNO3_R2_mean, color='orange', label=r'HNO$_3$', zorder=3)
plt.errorbar(X, HNO3_R2_mean, yerr=HNO3_R2_std, ecolor='black', label=r'Standard Deviation HNO$_3$', color='orange')
plt.plot(X, Pu_R2_mean, color='blue', label=r'Pu$^{+4}$', zorder=3)
plt.errorbar(X, Pu_R2_mean, yerr=Pu_R2_std, ecolor='black', label=r'Standard Deviation Pu$^{+4}$', color='blue')
plt.xlabel('Principal Components')
plt.ylabel(r'r$^2$ Score')
plt.title(r'r$^2$ vs. Principal Components' + f' - {MoR} MoR - {type}')
