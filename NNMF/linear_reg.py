...
from nnmf import nnmf, data, W_train, W_test
...
def linear_regression(concen_train, concen_test, W_train, W_test):
    LR_model = LinearRegression()
    LR_model.fit(W_train, concen_train)
    predicted_concen = LR_model.predict(W_test)
    return (predicted_concen, concen_test)

def display_Pu(nnmf_obj, predicted, test):
    ...
    error = f'Components Used: {nnmf_obj.n_components_}\n' 
    ...
    f = [(m*x+b) for x in X]
    plt.plot(X, f, label=error, color='palegreen')
...
def display_HNO3(nnmf_obj, predicted, test):
    ...
    m, b = polyfit(X, Y, deg=1)
    f = [(m*x+b) for x in X]
    plt.plot(X, f, label=error, color='orangered')
...
if __name__ == '__main__':
    # Change rank and data constraints in nnmf.py file.
    predicted, test = linear_regression(data.concen_train, data.concen_test, W_train, W_test)
    display_Pu(nnmf, predicted, test)
    display_HNO3(nnmf, predicted, test)